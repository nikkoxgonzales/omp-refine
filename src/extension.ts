...
interface UiLike {
  // Optional second arg: 0-based UTF-16 offset into the restored text where
  // the cursor should land (just after the spliced newline). Neither host
  // honors it today — omp 18.1.x types it `(text: string) => void` and pi
  // 0.85.x calls `editor.setText(text)` (cursor anchored to end) — so it is
  // ignored there (extra args are a no-op in JS) and parks forward-compat
  // placement for any host that adopts the convention.
 ...
 */
let pendingRawDraft: string | undefined;

function takePendingRawDraft(): string | undefined {
 ...
 */
export function createSubmitSnapshotTap(deps: SubmitSnapshotDeps): (data: string) => undefined {
  return (_data: string) => {
    try {
      const draft = deps.getText();
      if (typeof draft === 'string') pendingRawDraft = draft;
    } catch {
...
function armRefine(ctx: ContextLike): void {
  ...
  const combined = (data: string): { consume?: boolean } | undefined => {
    snapshot(data);
    return escape(data);
  };
  try {
